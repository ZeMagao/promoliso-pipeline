# Deploy ROLLBACK do carrossel variável: patch_rollback_carrossel.cjs.
# Para n8n -> backup DB (*.pre-rollback) -> restaura produtor+publicador dos backups (5 slides
# fixos + 6 children fixos) -> religa -> healthz. Preserva monitor (PRMLERR+watchdog).
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-rollback")) -Force } }
Write-Host 'backup feito (*.pre-rollback)'

$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $root 'design\patch_rollback_carrossel.cjs')
if ($LASTEXITCODE -ne 0) { throw 'patch_rollback_carrossel.cjs falhou' }
