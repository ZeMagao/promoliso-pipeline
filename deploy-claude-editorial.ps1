# Troca o modelo do agente editorial p/ Claude. EXIGE credencial Anthropic ja criada no editor.
# Para n8n, backup, roda writer_claude_editorial.cjs (auto-descobre a cred), religa.
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-claude-editorial")) -Force } }
...
$V = (Get-Content -LiteralPath (Join-Path $root 'newversion-claude.txt') -Raw).Trim()
