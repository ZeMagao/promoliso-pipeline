# Deploy alta resolução: patch_frases.cjs (scale:2 + quality:95 nos 5 nós de render).
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-frases")) -Force } }
Write-Host 'backup feito'

$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $root 'design\patch_frases.cjs')
if ($LASTEXITCODE -ne 0) { throw 'patch_frases.cjs falhou' }
