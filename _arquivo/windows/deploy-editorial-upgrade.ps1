# Deploy combinado: systemMessage enriquecido (patch_systemMessage.cjs) + swap GPT->Claude Sonnet 5 (writer_claude_editorial.cjs).
# Espelha deploy-claude-editorial.ps1: para n8n, backup, roda os 2 patches, religa, healthz.
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-editorial-upgrade")) -Force } }
Write-Host 'backup feito'

$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $root 'design\patch_systemMessage.cjs')
if ($LASTEXITCODE -ne 0) { throw 'patch_systemMessage.cjs falhou' }
& $nodeExe (Join-Path $root 'design\writer_claude_editorial.cjs')
if ($LASTEXITCODE -ne 0) { throw 'writer_claude_editorial.cjs falhou (credencial Anthropic existe?)' }
