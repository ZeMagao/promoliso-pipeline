# Deploy alta resolução: patch_render_hires.cjs (scale:2 + quality:95 nos 5 nós de render).
# O renderizador (:5680) e o arquivo do nó Sharp já foram editados em disco; este script
# só reinicia o n8n com o DB patchado pra carregar params novos + Sharp q95.
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-render-hires")) -Force } }
Write-Host 'backup feito'

$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $root 'design\patch_render_hires.cjs')
if ($LASTEXITCODE -ne 0) { throw 'patch_render_hires.cjs falhou' }
