# Deploy do design TITAN: writer.cjs patcha os 2 nos de render (Code in JavaScript / 1).
# Preserva editorial (Sonnet+sysMsg) e render hi-res (ja no DB). stop/backup/writer/religa.
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-titan-deploy")) -Force } }
Write-Host 'backup feito'

$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $root 'design\writer.cjs')
if ($LASTEXITCODE -ne 0) { throw 'writer.cjs falhou' }
Remove-Item Env:\NODE_PATH -ErrorAction SilentlyContinue
Write-Host 'design TITAN aplicado'
