# Deploy carrossel variável (PRINCIPAL): patch_carrossel.cjs.
# Para n8n -> backup DB (*.pre-carrossel) -> patch (prompt+capa+editfields+validador+fila+parser)
# -> religa reusando o túnel -> healthz. Só o PRINCIPAL (NL8eVLKErgnIXBQq); publicador é à parte.
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-carrossel")) -Force } }
Write-Host 'backup feito (*.pre-carrossel)'

$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $root 'design\patch_carrossel.cjs')
if ($LASTEXITCODE -ne 0) { throw 'patch_carrossel.cjs falhou' }
