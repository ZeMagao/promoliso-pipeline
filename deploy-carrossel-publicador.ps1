# Deploy carrossel variável (PUBLICADOR): patch_carrossel_publicador.cjs.
# Para n8n -> backup DB (*.pre-carrossel-pub) -> children dinâmicos no "Create a carousel post"
# -> religa -> healthz. Rodar SÓ depois de validar o carrossel curto na fila (step 2 do plano).
# Depois deste deploy: usuário dá Execute no "Executar Publicador" numa row READY curta e confere o IG.
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-carrossel-pub")) -Force } }
Write-Host 'backup feito (*.pre-carrossel-pub)'

$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $root 'design\patch_carrossel_publicador.cjs')
if ($LASTEXITCODE -ne 0) { throw 'patch_carrossel_publicador.cjs falhou' }
