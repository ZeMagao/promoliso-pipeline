# Deploy monitor Parte B (watchdog de falha silenciosa): insert_watchdog.cjs.
# Para n8n -> backup DB (*.pre-watchdog) -> insere workflow novo (active=1, cron 13:00+21:30 BRT)
# -> religa -> healthz. Restart ativa o schedule trigger do watchdog.
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-watchdog")) -Force } }
Write-Host 'backup feito (*.pre-watchdog)'

$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $root 'design\insert_watchdog.cjs')
if ($LASTEXITCODE -ne 0) { throw 'insert_watchdog.cjs falhou' }
Remove-Item Env:\NODE_PATH -ErrorAction SilentlyContinue
Write-Host 'insert aplicado'
