# Deploy fix #1 (publicador não estranda row): patch_publicador_errorhandling.cjs.
# Para n8n -> backup (*.pre-pub-eh) -> saída de erro nos nós de publish -> Marcar FALHA + email
# -> religa -> healthz. Preserva o resto. Passa ALERT_EMAIL (email de alerta) via env.
$ErrorActionPreference = 'Stop'
$root = 'C:\Users\Magal\Documents\Codex\promoliso-n8n'
$nodeExe = 'C:\Program Files\nodejs\node.exe'
$env:ALERT_EMAIL = '<email-de-alerta>'   # email de alerta (fora do repo; muda aqui se precisar)
...
if (Get-NetTCPConnection -LocalPort 5678 -State Listen -ErrorAction SilentlyContinue) { throw 'n8n ainda ouvindo na 5678 apos 20s — abortando (evita corromper DB)' }
Write-Host 'n8n parado'

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-pub-eh.$stamp")) -Force } }
Write-Host "backup feito (*.pre-pub-eh.$stamp)"

$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $root 'design\patch_publicador_errorhandling.cjs')
if ($LASTEXITCODE -ne 0) { throw 'patch_publicador_errorhandling.cjs falhou' }
...
if($ok){Write-Host "n8n UP"}else{throw 'n8n nao subiu (sem healthz)'}
Write-Host 'DONE'
