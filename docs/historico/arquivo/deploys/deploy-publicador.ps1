# Insere o workflow Publicador (fila) + seed. Para n8n, backup, insere, religa. active=0 (so manual).
...
foreach ($ext in @('', '-wal', '-shm')) {
  ...
  if (Test-Path -LiteralPath $srcf) { Copy-Item -LiteralPath $srcf -Destination (Join-Path $root ("backups\database.sqlite$ext.pre-publicador")) -Force }
}

Write-Host 'inserir workflow + seed'
$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $scratch 'insert_publicador.cjs')
if ($LASTEXITCODE -ne 0) { throw 'insert_publicador.cjs falhou' }
...
if ($ok) { Write-Host 'n8n UP' } else { Write-Warning 'sem healthz em 60s; ver n8n.stderr.log' }
Write-Host 'DONE'
