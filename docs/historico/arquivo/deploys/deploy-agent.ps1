# Deploy do campo subtitulo no agente/parser. Mesmo metodo do deploy-redesign.ps1.
...
foreach ($ext in @('', '-wal', '-shm')) {
  ...
  if (Test-Path -LiteralPath $srcf) { Copy-Item -LiteralPath $srcf -Destination (Join-Path $root ("backups\database.sqlite$ext.pre-agent-subtitulo")) -Force }
}

Write-Host 'gravar (writer2.cjs)'
$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $scratch 'writer2.cjs')
if ($LASTEXITCODE -ne 0) { throw 'writer2.cjs falhou' }
