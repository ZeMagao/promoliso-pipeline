# Fix da capa (overflow titulo/destaque). Patcha nos de render via writer.cjs. Mesmo metodo.
...
foreach ($ext in @('', '-wal', '-shm')) { $s=Join-Path $root ("data\.n8n\database.sqlite$ext"); if (Test-Path $s){ Copy-Item $s (Join-Path $root ("backups\database.sqlite$ext.pre-capafix")) -Force } }
$env:NODE_PATH = Join-Path $root 'node_modules'
& $nodeExe (Join-Path $scratch 'writer.cjs'); if ($LASTEXITCODE -ne 0) { throw 'writer falhou' }
